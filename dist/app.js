 ...
 */
const errorHandler = (socket, event, msg) => {
...
io.on("connection", (socket) => {
    ...
    socket.on("join_server", (username) => {
        if (username === "") {
            errorHandler(socket, "join_server", "The username is invalid. Please select a different one.");
        }
        else if (users.has(username)) {
            errorHandler(socket, "join_server", "The username is already in use! Please choose a different username.");
        }
        else {
    ...
    socket.on("create_room", (username, roomName) => {
        if (roomAdmins.has(roomName)) {
            errorHandler(socket, "create_room", "The room name is already in use! Please choose a different room name.");
        }
        else {
     ...
     */
    socket.on("delete_room", (username, roomName) => {
        var _a;
        if (roomAdmins.get(roomName) !== username) {
            errorHandler(socket, "delete_room", "You are not authorized to delete the room.");
        }
        else {
            ...
            if (room) {
                ...
                for (const socketId of room) {
                    (_a = io.sockets.sockets.get(socketId)) === null || _a === void 0 ? void 0 : _a.leave(roomName);
                }
            }
    ...
    socket.on("join_room", (username, roomName) => {
        if (!roomAdmins.has(roomName)) {
            errorHandler(socket, "join_room", "Invalid Room Name! Please choose a different room.");
        }
        else if (roomMembers[roomName].includes(username)) {
            errorHandler(socket, "join_room", "You are already a member of this room!");
        }
        else {
    ...
    socket.on("leave_room", (username, roomName) => {
        if (!roomAdmins.has(roomName)) {
            errorHandler(socket, "leave_room", "Invalid Room Name! Please choose a different room.");
        }
        else if (!roomMembers[roomName].includes(username)) {
            errorHandler(socket, "leave_room", "You are not a member of this room!");
        }
        else {
            // Remove the user from the roomMembers list and make them leave the room
            roomMembers[roomName].splice(roomMembers[roomName].indexOf(username), 1);
     ...
     */
    socket.on("send_message", (username, roomName, content) => {
        if (!messages[roomName]) {
            errorHandler(socket, "send_message", "Invalid Room Name! Please choose a different room.");
        }
        else if (!roomMembers[roomName].includes(username)) {
            errorHandler(socket, "send_message", "You are not a member of this room!");
        }
        else {
            ...
            socket.broadcast.to(roomName).emit("new_message", Object.assign({ success: true, roomName: roomName }, payload));
